...
function hasHttpCap(nodes: Node<AnyNodeData>[]) {
  return nodes.some(
    (n) => n.data.kind === "cap.http.get" || n.data.kind === "cap.http.post"
  );
}
...
function generateCapabilityCode(
  ...
  const site = refs.find((r) => r.data.kind === "website")?.data as WebsiteData | undefined;
...
function generateWorkflowTs(g: WorkflowGraph): string {
  ...
  const hasHttp = hasHttpCap(g.nodes);
...
function generateWorkflowYaml(name: string, description: string): string {
  const s = slug(name);
